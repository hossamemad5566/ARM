...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
	...
	if (   (htim == &htim4)  && htim->Channel == HAL_TIM_ACTIVE_CHANNEL_4)  // if the interrupt source is channel1
	{
		if (HCSR04_sensor.Is_First_Captured==0) // if the first value is not captured
		{
			/*														edit channel	*/
			HCSR04_sensor.IC_Val1 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_4); // read the first value
		...
		{
			/*														edit channel	*/
			HCSR04_sensor.IC_Val2 = HAL_TIM_ReadCapturedValue(htim, TIM_CHANNEL_4);  // read second value
